...
def makespan(opr_MA,opr_OS,num_machine,num_operation,benchmark,num_job,opr_list):   #(機械割当て遺伝子列、操作順序遺伝子列、機械数、操作数、工程表、仕事数、操作数の区切り)
    ...
    for i in range(num_operation):
        if machine_available_time[opr_MA[i]]>job_available_time[opr_OS[i]]:     #機械の操作可能開始時間>仕事の割当て開始可能時間
            operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][0] = machine_available_time[opr_MA[i]]
            operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][1] = machine_available_time[opr_MA[i]]+benchmark[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][opr_MA[i]]
            operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][2] = opr_MA[i]
        else:                                                                   #機械の操作可能開始時間<仕事の割当て開始可能時間
            operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][0] = job_available_time[opr_OS[i]]
            operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][1] = job_available_time[opr_OS[i]]+benchmark[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][opr_MA[i]]
            operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][2] = opr_MA[i]
        machine_available_time[opr_MA[i]] = operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][1]
        job_available_time[opr_OS[i]] = operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][1]
...
def elitizm(gene_os,gene_ma,makespan,N_gene):
    ...
    for i in range(N_gene):
        ...
        # for j in range(int(N_gene/10)):
            # if list_elite[j]>makespan[i]:           #個体のメイクスパンが暫定のエリート個体よりメイクスパンが小さければ代入
                # list_elite[j] = makespan[i]
                # list_elite_index[j] = i
                # break
...
def ganttchart(opr_MA,opr_OS,num_machine,num_operation,benchmark,num_job,opr_list):   #(機械割当て遺伝子列、操作順序遺伝子列、機械数、操作数、工程表、仕事数、操作数の区切り)
    ...
    for i in range(num_operation):
        if machine_available_time[opr_MA[i]]>job_available_time[opr_OS[i]]:     #機械の操作可能開始時間>仕事の割当て開始可能時間
            ...
            operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][1] = job_available_time[opr_OS[i]]+benchmark[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][opr_MA[i]]
            operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][2] = opr_MA[i]
        machine_available_time[opr_MA[i]] = operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][1]     #機械の操作可能時間の更新
        job_available_time[opr_OS[i]] = operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][1]         #仕事の割当て可能時間の更新
        plt.barh(y=operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][2],width=benchmark[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][opr_MA[i]],left=operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][0],color=cmap(opr_OS[i]),ec='k')    #プロット
        plt.text((operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][0]+operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][1])/2,operation_graph[opr_list[opr_OS[i]]+operaton_cnt[opr_OS[i]]][2],opr_OS[i],ha="center")
...
f = open('MK10.txt','r',encoding='utf-8')    #ファイル読み込み
# print(f)
mat = []                                    #ベンチマーク問題
...
for h in range(10):
    ...
    # plt.ylabel("makespan")
    # plt.show()                      #描画の出力
    g = makespan_gene.index(min(makespan_gene))     #最小メイクスパンの個体のインデックスを求める
    # ganttchart(population_MA[g],population_OS[g],N_machine,sum_opr,mat_ope,N_job,opr_list)      #ガントチャートの作成